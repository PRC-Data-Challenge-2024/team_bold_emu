...
challenge_set = pd.read_csv('final_submission_set.csv')

# Load the additional datasets
ground_speed_data = pd.read_csv('ground_speed_data_final_submission_set.csv')
jet_stream_data = pd.read_csv('jet_stream_data_final_submission_set.csv')
weather_data = pd.read_csv('weather_data_final_submission_set.csv')
...
# Drop rows with missing values, excluding the 'tow' column
challenge_set = challenge_set.dropna(subset=[col for col in challenge_set.columns if col != 'tow'])
...
challenge_set.to_csv('final_submission_set_v6.csv', index=False)

print("Data successfully merged and saved as 'final_submission_set_v6.csv'")
